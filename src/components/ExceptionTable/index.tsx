...
interface ExceptionTableProps {
  ...
  onStoreManagerFilterChange?: (v: string) => void
  onCsViewFilterChange?: (v: string) => void
...
export const ExceptionTable = React.memo(function ExceptionTable({
  ...
  storeManagerFilter: externalStoreManagerFilter,
  csViewFilter: externalCsViewFilter,
  ...
  onStoreManagerFilterChange,
  onCsViewFilterChange,
  ...
  const storeManagerFilter = externalStoreManagerFilter ?? '全部'
  const csViewFilter = externalCsViewFilter ?? '全部'
  ...
  const setStoreManagerFilter = onStoreManagerFilterChange || (() => {})
  const setCsViewFilter = onCsViewFilterChange || (() => {})
  ...
  const filteredRecords = useMemo(() => {
    const result = records.filter((record) => {
      ...
      // 店铺管理员筛选：单元格按逗号拆分，列表包含选中管理员即命中（一个管理员管多店铺都会筛出）
      if (storeManagerFilter !== '全部') {
        const list = record.storeManagers.split(/[,，]/).map(s => s.trim())
        if (!list.includes(storeManagerFilter)) return false
      }
      // 账号分类筛选（前端单选，精确匹配）
      if (csViewFilter !== '全部' && record.isCustomerService !== csViewFilter) return false
      // 关键词搜索：编号 + 业务渠道指导员 + 申请人 + 店铺管理员
      if (searchKeyword) {
        ...
        return (
          ...
          record.storeManagers.toLowerCase().includes(keyword)
        )
      }
      return true
    })
  ...
  }, [records, statusFilter, riskFilter, platformFilter, exceptionTypeFilter, storeManagerFilter, csViewFilter, searchKeyword])

  // 导出 Excel：xlsx 动态加载，导出期间禁用按钮
  ...
  // 动态店铺管理员选项（从记录的 storeManagers 按逗号拆分去重，按名称排序）
  const storeManagerOptions = useMemo(() => {
    const unique = [...new Set(
      records.flatMap((r) => r.storeManagers.split(/[,，]/).map((s) => s.trim()).filter(Boolean))
    )].sort((a, b) => a.localeCompare(b, 'zh-CN'));
    return [
      { label: '管理员', value: '全部' },
      ...unique.map((m) => ({ label: m, value: m })),
    ]
  }, [records])

  // 动态「账号分类」选项（从当前记录提取，状态筛选已移交上方卡片，此处仅账号分类下拉）
  const csViewOptions = useMemo(() => {
    const unique = [...new Set(records.map((r) => r.isCustomerService).filter(Boolean))];
    return [
      { label: '账号分类', value: '全部' },
  ...
  return (
    ...
    <div className="space-y-2">
      {/* 筛选栏 */}
      <div className="flex flex-nowrap items-center gap-2 overflow-x-auto rounded-lg border border-[#e5e6eb] bg-white p-2">
        <div className="flex items-center gap-1.5 text-sm text-[#646a73] pr-1 shrink-0">
          <Filter className="h-4 w-4" />
          <span>筛选</span>
        </div>

        <Select value={csViewFilter} onValueChange={handleFilterChange(setCsViewFilter)}>
          <SelectTrigger className="h-8 w-[100px] shrink-0">
            <SelectValue placeholder="账号分类" />
          </SelectTrigger>
          <SelectContent>
            {csViewOptions.map((opt) => (
        ...
        <Select value={storeManagerFilter} onValueChange={handleFilterChange(setStoreManagerFilter)}>
          <SelectTrigger className="h-8 w-[88px] shrink-0">
            <SelectValue placeholder="管理员" />
          </SelectTrigger>
          <SelectContent className="max-h-[220px]">
            {storeManagerOptions.map((opt) => (
              <SelectItem key={opt.value} value={opt.value}>
                {opt.label}
              </SelectItem>
            ))}
        ...
        <a
          ...
          className="text-sm text-[#3370ff] hover:underline shrink-0"
        >
          指引
        </a>
        ...
        <div className="text-sm text-[#646a73] whitespace-nowrap shrink-0">
        ...
        <Button
          variant="outline"
          size="sm"
          className="h-8 px-3 text-xs gap-1.5 whitespace-nowrap shrink-0"
