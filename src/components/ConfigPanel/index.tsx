...
const fieldGroups: { title: string; fields: { label: string; key: keyof IPluginConfig }[] }[] = [
  {
    ...
    fields: [
      ...
      { label: '通知时间（超时判断依据）', key: 'noticeTimeFieldId' },
    ],
  },
  {
    title: '权限中心',
...
const ConfigPanel = React.memo(function ConfigPanel({ config, setConfig, onSave }: Props) {
  ...
  // 缓存：tableId+csFid → 账号分类列表，避免重复全表扫描
  const csCacheRef = useRef<Map<string, string[]>>(new Map());
  const csLoadingRef = useRef(false);
  ...
  useEffect(() => {
    ...
    csCacheRef.current.clear();    // 切换多维表格，清空账号分类缓存
  }, [config.baseToken]);
  ...
  // 读取主表记录 → 提取「账号分类」列表用于筛选（带缓存 + 防并发）
  useEffect(() => {
    if (!workspaceBase || !config.tableId || !config.isCustomerServiceFieldId) {
      setCsList([]);
      return;
    }
    const cacheKey = `${config.tableId}|${config.isCustomerServiceFieldId}`;
    const cached = csCacheRef.current.get(cacheKey);
    if (cached) {
      setCsList(cached);
      return;
    }
    if (csLoadingRef.current) return;
    csLoadingRef.current = true;

    let cancelled = false;
    const csFid = config.isCustomerServiceFieldId;
    workspaceBase.getTable(config.tableId).then(async (table: any) => {
      try {
        ...
        do {
          ...
          const records = res?.records ?? [];
          for (const r of records) {
            const text = cellToText(r.fields?.[csFid]).trim();
            if (text) csSet.add(text);
          }
          page++;
          pageToken = res?.hasMore ? (res.pageToken ?? undefined) : undefined;
        } while (pageToken !== undefined && page < 100);
        if (cancelled) return;
        const sorted = [...csSet].sort((a, b) => a.localeCompare(b, 'zh-CN'));
        csCacheRef.current.set(cacheKey, sorted);
        setCsList(sorted);
      } catch {
        if (!cancelled) setCsList([]);
      } finally {
        csLoadingRef.current = false;
      }
    }).catch(() => {
      if (!cancelled) setCsList([]);
      csLoadingRef.current = false;
    ...
    return () => { cancelled = true; };
  }, [workspaceBase, config.tableId, config.isCustomerServiceFieldId]);
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-[#646a73]">
                      账号分类筛选（从主表读取）
                      {csList.length === 0 && config.tableId && config.isCustomerServiceFieldId && (
                        <span className="text-[#8f959e] ml-1">— 加载中...</span>
                      )}
                    </label>
                    {(() => {
                      const selected = (config.isCustomerServiceFilter || '').split(',').map(s => s.trim());
                      const selectedSet = new Set(selected);
                      return csList.map((cs) => {
                        const isChecked = selectedSet.has(cs);
                        return (
                          <label key={cs} className="flex items-center gap-2 rounded px-1 py-0.5 cursor-pointer hover:bg-[#f5f6f8]">
                            <input
                              type="checkbox"
                              checked={isChecked}
                              onChange={() => {
                                const current = selected.filter(Boolean);
                                const next = isChecked
                                  ? current.filter(d => d !== cs)
                                  : [...current, cs];
                                setConfig({ ...config, isCustomerServiceFilter: next.join(',') });
                              }}
                              className="h-3.5 w-3.5 rounded border-[#c9ccd0] text-[#3370ff] focus:ring-[#3370ff]"
                            />
                            <span className="text-xs text-[#1f2329]">{cs}</span>
                          </label>
                        );
                      });
                    })()}
                    {(config.isCustomerServiceFilter || '').split(',').filter(Boolean).length > 0 && (
                      <button
                        onClick={() => setConfig({ ...config, isCustomerServiceFilter: '' })}
                        className="text-xs text-[#3370ff] hover:underline mt-1"
                      >
                        清除筛选
                      </button>
