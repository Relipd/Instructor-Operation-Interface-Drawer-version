...
function parseStatus(val: string): ExceptionRecord['status'] {
  ...
  // 排除风险 / 整改完结 / 系统RPA填充 / 已完成 / 已完结 → 已完结
  if (val === 'completed' || val === '已完成' || val === '已完结' || val === 'resolved' || val === '已解决' || val === '排除风险' || val === '整改完结' || val === '系统RPA填充') return '已完结';
  return '待处理';
}
...
// 系统自动完结的异常类型（仅这两种，不含 +角色不一致 的组合）
const SYSTEM_CLOSE_EXCEPTION_TYPES = [
  '辅助信息不一致：账号名（昵称）不一致',
  '辅助信息不一致：账号名（姓名）不一致',
];
...
function calcTimeout(noticeTime: string, deadlineText: string, status: ExceptionRecord['status']): boolean {
  if (status === '已完结' || status === '整改中') return false;
  if (!deadlineText || deadlineText === '-') return false;
  if (!noticeTime) return false;

  // noticeTime 形如 'yyyy-mm-dd'。补 'T00:00:00' 强制按本地时区解析，
  // 否则 new Date('yyyy-mm-dd') 会被当成 UTC 00:00，在东八区变成本地 08:00 造成偏差。
  const noticeDate = new Date(noticeTime + 'T00:00:00');
  ...
  // 按工作日累加 N 天，跳过周六周日（deadlineText 语义为「工作日」）
  // 例：周一 +1工作日 = 周二；周五 +3工作日 = 周三（跨周末）
  const deadline = new Date(noticeDate);
  let workdaysLeft = deadlineDays;
  while (workdaysLeft > 0) {
    deadline.setDate(deadline.getDate() + 1);
    const dow = deadline.getDay(); // 0=周日, 6=周六
    if (dow !== 0 && dow !== 6) workdaysLeft--;
  }
  // 截止点取「第 N 个工作日的 00:00」：第 N 个工作日当天即视为超时
  // 例：1工作日 → 次工作日 00:00 起即超时（周一通知，周二当天就显示超时）
...
function mapFeishuRecord(r: any, cfg: IPluginConfig, index: number): ExceptionRecord {
  ...
  // 审阅时间：统一格式化为 yyyy-mm-dd（兼容 yyyy/mm/dd 与 yyyy-mm-dd，过滤 1900 等异常值）
  const reviewTimeRaw = cellToDate(r.fields[cfg.reviewTimeFieldId] ?? '');
  let reviewTime = reviewTimeRaw ? reviewTimeRaw.slice(0, 10) : '';
  if (reviewTime) {
    const year = parseInt(reviewTime.slice(0, 4), 10);
    if (isNaN(year) || year < 1971) reviewTime = '';  // 1900/01/01、时间戳0 等视为无效
  }

  // 通知时间：超时判断依据，同样格式化为 yyyy-mm-dd
  const noticeTimeRaw = cellToDate(r.fields[cfg.noticeTimeFieldId] ?? '');
  let noticeTime = noticeTimeRaw ? noticeTimeRaw.slice(0, 10) : '';
  if (noticeTime) {
    const year = parseInt(noticeTime.slice(0, 4), 10);
    if (isNaN(year) || year < 1971) noticeTime = '';
  }

  // 账号分类（动态读取去重）
  const isCustomerService = cellToText(r.fields[cfg.isCustomerServiceFieldId] ?? '');
  ...
  return {
    ...
    storeManagers: cellToText(r.fields[cfg.storeManagersFieldId] ?? ''),      // 店铺管理员（逗号分隔多个）
...
export default function App() {
  ...
  const [storeManagerFilter, setStoreManagerFilter] = useState<string>('全部');
  const [csViewFilter, setCsViewFilter] = useState<string>('全部');
  ...
  const systemWriteRef = useRef(false);   // 系统RPA填充回传进行中标记，防 onDataChange 循环
  ...
  const fetchData = useCallback(async (cfg: IPluginConfig) => {
    ...
    try {
      ...
      // 动态计算超时 + 识别系统自动完结（一次遍历，原地更新）
      const timeoutRecordIds: string[] = [];
      const systemCloseTargets: string[] = [];  // 需系统RPA填充回传的 recordId
      for (let i = 0; i < allMapped.length; i++) {
        const r = allMapped[i];
        // 超时计算
        const timeout = calcTimeout(r.noticeTime, r.deadlineText || '', r.status);
        ...
        // 系统自动完结：仅待处理 + 命中指定异常类型（不含 +角色不一致 组合）
        if (r.status === '待处理' && SYSTEM_CLOSE_EXCEPTION_TYPES.includes((r.exceptionType || '').trim())) {
          allMapped[i] = {
            ...allMapped[i],
            status: '已完结',
            riskAction: '系统RPA填充',
          ...
          systemCloseTargets.push(r.recordId);
        }
      }
      ...
      // 系统RPA填充回传：待处理 + 指定异常类型 → 自动写「系统RPA填充」+ 反馈信息
      // 串行在超时回传之后（避免并发写同一 record 的 setRecord 竞态）；用独立 ref 防循环
      if (cfg.statusFieldId && systemCloseTargets.length > 0 && !systemWriteRef.current) {
        systemWriteRef.current = true;
        const CONCURRENCY = 5;
        for (let i = 0; i < systemCloseTargets.length; i += CONCURRENCY) {
          const chunk = systemCloseTargets.slice(i, i + CONCURRENCY);
          await Promise.all(chunk.map(rid => {
            const fields: Record<string, any> = {};
            fields[cfg.statusFieldId] = '系统RPA填充';
            if (cfg.feedbackStatusFieldId) fields[cfg.feedbackStatusFieldId] = '系统RPA填充';
            if (cfg.feedbackResultFieldId) fields[cfg.feedbackResultFieldId] = '由RPA后台自动填充';
            if (cfg.feedbackTimeFieldId) fields[cfg.feedbackTimeFieldId] = Date.now();
            if (cfg.feedbackPersonFieldId) fields[cfg.feedbackPersonFieldId] = '系统RPA填充';
            if (cfg.feedbackIsTimeoutFieldId) fields[cfg.feedbackIsTimeoutFieldId] = '否';
            return updateRecordFields(cfg.tableId, rid, fields)
              .catch(e => console.warn('系统RPA填充回传失败:', rid, e));
          }));
        }
        // 回传完成后延时清 flag，让下次 onDataChange 能正常处理
        setTimeout(() => { systemWriteRef.current = false; }, 3000);
      }
    } catch (e) {
  ...
  const dataConfigKey = `${config.tableId}|${config.statusFieldId}|${config.platformFieldId}|${config.dataPlatformNameFieldId}|${config.handlerFieldId}|${config.exceptionDetailFieldId}|${config.departmentFieldId}|${config.isCustomerServiceFieldId}|${config.reviewTimeFieldId}|${config.noticeTimeFieldId}|${config.exceptionCodeFieldId}`;
  useEffect(() => {
  ...
  const records = useMemo(() => {
    ...
    if (config.isCustomerServiceFilter) {
      const selectedCs = config.isCustomerServiceFilter.split(',').map(s => s.trim().toLowerCase()).filter(Boolean);
      if (selectedCs.length > 0) {
        result = result.filter(r => selectedCs.includes((r.isCustomerService || '').toLowerCase()));
      }
    }
    return result;
  }, [allRecords, config.handlerNameFilter, config.departmentNameFilter, config.isCustomerServiceFilter]);
  ...
  useEffect(() => {
    ...
    const off = activeDashboard.onDataChange(() => {
      ...
      if (timeoutWriteRef.current || systemWriteRef.current) return;
        ...
        <div className="flex-1 min-w-0">
          <main className="mx-auto max-w-[1400px] space-y-3 p-4">
            ...
            <ExceptionTable
              ...
              storeManagerFilter={storeManagerFilter}
              csViewFilter={csViewFilter}
              searchKeyword={searchKeyword}
              onStatusFilterChange={setStatusFilter}
              onRiskFilterChange={setRiskFilter}
              onPlatformFilterChange={setPlatformFilter}
              onExceptionTypeFilterChange={setExceptionTypeFilter}
              onStoreManagerFilterChange={setStoreManagerFilter}
              onCsViewFilterChange={setCsViewFilter}
