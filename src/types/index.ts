export interface ExceptionRecord {
  ...
  department: string;
  isCustomerService: string;        // 账号分类（动态读取）
  reviewTime: string;
  noticeTime: string;               // 通知时间（超时判断依据，纯展示）
  isTimeout: boolean;
  deptInstructors: string;          // 业务渠道指导员（主表字段，用于搜索）
  storeManagers: string;            // 店铺管理员（主表字段，逗号分隔多个，用于筛选）
  // 整改相关
...
export interface IPluginConfig {
  ...
  noticeTimeFieldId: string;            // 通知时间（超时判断依据）
  departmentFieldId: string;            // 归属部门
  isCustomerServiceFieldId: string;     // 账号分类
  deptInstructorsFieldId: string;       // 业务渠道指导员
  sharedInstructorsFieldId: string;     // 支持部门指导员
  storeManagersFieldId: string;         // 店铺管理员（逗号分隔多个）
  // 权限中心字段
  ...
  isCustomerServiceFilter: string;
}
